...
### awsext_lexv2_bot_alias
- File: provider/lexv2_bot_alias.go
- APIs: CreateBotAlias (C), DescribeBotAlias (R), UpdateBotAlias (U), DeleteBotAlias (D)
- adopt_on_exists write-only flag to adopt an existing alias instead of erroring; optional CloudWatch text logging (text_log_enabled, text_log_cw_log_group_arn, text_log_prefix); import by <bot_id>/<bot_alias_id>

### awsext_lexv2_resource_policy
- File: provider/lexv2_resource_policy.go
- APIs: CreateResourcePolicy (C), DescribeResourcePolicy (R), UpdateResourcePolicy (U), DeleteResourcePolicy (D)
- Attaches a JSON resource-based policy to a Lex V2 bot or bot alias; adopts on PreconditionFailedException (policy already exists); import by resource_arn

### awsext_connect_bot_association
- File: provider/connect_bot_association.go
- APIs: AssociateBot (C), ListBots (R), DisassociateBot (D)
- Associates a Lex V2 bot alias with a Connect instance (fills gap left by aws_connect_bot_association which only supports Lex V1); all fields RequiresReplace; import by <instance_id>/<alias_arn>

### awsext_bedrockagentcore_gateway
- File: provider/bedrockagentcore_gateway.go
- APIs: CreateGateway (C), GetGateway (R), UpdateGateway (U), DeleteGateway (D)
- CUSTOM_JWT / MCP gateway; bootstrap audience workaround (create with placeholder, immediately patch allowed_audience to gateway_id); adopt-on-ConflictException by name lookup; import by gateway_id
- Schema: gateway_id/gateway_arn/gateway_url/status (computed), name (required, RequiresReplace), description/role_arn/exception_level/kms_key_arn (optional, updatable), protocol_type/authorizer_type (optional, RequiresReplace, default MCP/CUSTOM_JWT), custom_jwt_authorizer nested (discovery_url), allowed_audience list (computed), tags
...
provider/
  ...
  connect_bot_association.go           - awsext_connect_bot_association
  connect_tags.go                      - Shared tag helpers for Connect resources
  lexv2_bot_import.go
  lexv2_bot_locale_build.go
  lexv2_bot_alias.go                   - awsext_lexv2_bot_alias
  lexv2_resource_policy.go             - awsext_lexv2_resource_policy
  lexv2_tags.go                        - Shared tag helpers for LexV2 resources
  bedrockagentcore_gateway.go          - awsext_bedrockagentcore_gateway
